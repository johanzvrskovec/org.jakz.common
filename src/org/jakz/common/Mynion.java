 ...
 * @author johan
 */
public class Mynion implements Runnable, Callable<List<Workable>>, Serializable
{
	...
	private HashMap<Integer, Mynion> child;
	private Mynion parent;
	...
	private void init()
	{
		lock.writeLock().lock();
			...
			child = new HashMap<Integer, Mynion>();
	...
	public Mynion(String nSessionID, Logger nLog) 
	{
	...
	private Mynion(String nSessionID, int nChildID, Logger nLog) 
	{
		lock = new ReentrantReadWriteLock();
		init();
		lock.writeLock().lock();
	 ...
	 * @return this
	 */
	public Mynion setExecutor(ExecutorService nExecutor)
	{
		lock.writeLock().lock();
			executor=nExecutor;
		lock.writeLock().unlock();
	...
	public Mynion setSharedStore(SharedStore<String,Object> nSharedStore)
	{
		lock.writeLock().lock();
	...
	public Mynion setParent(Mynion nParent)
	{
		lock.writeLock().lock();
			parent=nParent;
		lock.writeLock().unlock();
	 ...
	 * @return this
	 */
	public Mynion setParallelism(Integer nParallelism)
	{
		lock.writeLock().lock();
	...
	public Mynion setRemote(boolean nRemote)
	{
		lock.writeLock().lock();
			remote=nRemote;
		lock.writeLock().unlock();
	...
	public Mynion setIntegritySecret(String nIntegritySecret)
	{
		lock.writeLock().lock();
	...
	/**
	 * Gets the number of {@link Mynion}s under this object's control. 
	 * @return the number
	 */
	...
	private int claimNewChildID() throws MynionException
	{
		lock.readLock().lock();
		...
		{
			lock.readLock().unlock();
			throw new MynionException("Can't create new worker id - workerIdIndex is at maximum.");
		}
		else
		{
			lock.writeLock().lock();
	...
	/**
	 * Spawns a new {@link Mynion} based on this object. The id of the new object is set according to this object's id template cursor. The newly created {@link Mynion} uses the same {@link ExecutorService} as this object.
	 * @return THE NEW {@link Mynion}
	 * @throws MynionException 
	 */
	public Mynion spawnLocalChild() throws MynionException
	{
		int newId=claimNewChildID();
		Mynion nw = new Mynion(sessionID,newId,log).setExecutor(executor).setSharedStore(sharedStore);
		putChild(nw);
		nw.submit();
		return nw;
	}
	
	private Mynion putChild(Mynion newChild) throws MynionException
	{
		if(newChild.sessionID!=sessionID)
			throw new MynionException("Child session ID not compatible with parent session ID");
		...
		{
			lock.writeLock().unlock();
			throw new MynionException("Duplicate ParallelWorker id in children tree. Id:"+newChild.childID);
		}
		lock.writeLock().unlock();
		return this;
	}
	...
	/**
	 * Submits this {@link Mynion} to work in it's own thread using the set {@link ExecutorService}.
	 */
	public Mynion submit()
	{	
		lock.writeLock().lock();
			runSwitch=true;
	...
	public Mynion stop() throws WorkableFailureException
	{
		lock.writeLock().lock();
			runSwitch=false;
		lock.writeLock().unlock();
	...
	public Mynion waitForDone(long timeoutMillis) throws InterruptedException, TimeoutException
	{
	...
	/**
	 * Gets the work assigned to this {@link Mynion}. <br><strong>Warning: Manipulating or reading internal parts can have unforeseen consequences when running threaded work. Make sure to lock the object you are using.</strong>
	 * @return the work
	 */
	...
	/**
	 * Assigns work to this {@link Mynion}.
	 * @param ntask
	 * @return
	 */
	public Mynion setWork(List<Workable> ntask)
	{
		lock.writeLock().lock();
			workQueue=ntask;
		lock.writeLock().unlock();
	...
	/**
	 * Adds work to this {@link Mynion}.
	 * @param ntask
	 * @return
	 */
	public Mynion addWork(Workable ntask)
	{
		lock.writeLock().lock();
			workQueue.add(ntask);
		lock.writeLock().unlock();
	...
	private void spawnLocalChildren() throws MynionException
	{
		lock.writeLock().lock();
		int i=0;
	...
	private void administerWork() throws MynionException, InterruptedException, WorkableFailureException
	{
		Mynion currentParent;
	...
	/**
	 * Runs this {@link Mynion} according to the implemented {@link Runnable} interface.
	 */
	@Override
	public void run()
	{
	...
	/**
	 * Calls this {@link Mynion} according to the implemented {@link Callable} interface.
